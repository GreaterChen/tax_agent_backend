...
@refundsController.post('/audit', dependencies=[Depends(CheckUserInterfaceAuth('system:refunds:edit'))])
@ValidateFields(validate_model='refund_audit')
@Log(title='退款审核', business_type=BusinessType.UPDATE)
async def audit_system_refunds(
    request: Request,
    refund_audit: RefundAuditModel,
    query_db: AsyncSession = Depends(get_db),
    current_user: CurrentUserModel = Depends(LoginService.get_current_user),
):
    audit_result = await RefundsService.audit_refunds_services(request, query_db, refund_audit, current_user.user.user_name)
    logger.info(audit_result.message)

    return ResponseUtil.success(msg=audit_result.message)
